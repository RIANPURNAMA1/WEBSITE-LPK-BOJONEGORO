      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-8">
        ...
        <motion.div variants={container} className="grid lg:grid-cols-2 gap-8">
          <motion.div variants={fadeUp} className="border border-slate-200 rounded-md p-7">
            ...
            <div className="space-y-6">
              {[
                { icon: Phone, idx: 0, href: "tel:+6281234567890", value: "+62 812-3456-7890" },
                { icon: MessageCircle, idx: 1, href: "https://wa.me/6281234567890", value: "+62 812-3456-7890" },
                { icon: Mail, idx: 2, href: "mailto:info@lpkbojonegoromendunia.com", value: "info@lpkbojonegoromendunia.com" },
                { icon: MapPin, idx: 3, href: null as string | null, value: "Jl. Madrasah, RT.03/RW.05, Gempol, Sidodadi, Kec. Sukosewu, Kabupaten Bojonegoro" },
              ].map(({ icon: Icon, idx, value, href }) => (
                <div key={idx} className="flex items-start gap-3">
                  ...
                  <div>
                    <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mb-0.5">
                      {t(`contact.info.items.${idx}.label`)}
                    </div>
                    {href ? (
                      <a
                        href={href}
          ...
          <motion.div variants={fadeUp} className="border border-slate-200 rounded-md p-7">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                    {t("contact.form.fields.0.label")} <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                    {t("contact.form.fields.1.label")} <span className="text-red-400">*</span>
                  </label>
                  <input
                    type="tel"
              ...
              <div>
                ...
                >
                  <option value="">{t("contact.form.fields.2.options.0") || "Pilih Program"}</option>
                  <option value="gakkou">{t("contact.form.fields.2.options.1")}</option>
                  <option value="tokutei">{t("contact.form.fields.2.options.2")}</option>
                  <option value="care">{t("contact.form.fields.2.options.3")}</option>
                  <option value="hospitality">{t("contact.form.fields.2.options.4")}</option>
                </select>
              </div>
              <div>
                <label className="block text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1.5">
                  {t("contact.form.fields.3.label")}
                </label>
                <textarea
                  rows={4}
                  placeholder={t("contact.form.fields.3.placeholder")}
                  value={message}
        ...
        <motion.div variants={fadeUp} className="mt-8 rounded-md overflow-hidden border border-slate-200">
          <div className="px-5 py-3 border-b border-slate-100 flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-700">{t("contact.map.title")}</span>
          </div>
          <iframe
